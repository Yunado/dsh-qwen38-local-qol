...
function toDraft(value) {
  ...
  const line = (name2) => ({
    baseURL: value.lines?.[name2]?.baseURL ?? "",
    model: value.lines?.[name2]?.model ?? "",
    displayName: value.lines?.[name2]?.displayName ?? ""
  });
  const active = legacy ? { baseURL: value.baseURL ?? "", model: value.model ?? "", displayName: value.displayName ?? "" } : line(dialect);
...
function QwenLocalSectionEntry({ useLocale, load, save }) {
  ...
  const doSave = async () => {
    ...
    const patch = {
      ...
      lines: {
        [draft2.dialect]: { baseURL: draft2.baseURL, model: draft2.model, displayName: draft2.displayName },
        [otherDialect]: { baseURL: draft2.parkedBaseURL, model: draft2.parkedModel, displayName: draft2.parkedDisplayName }
      },
